...
public class StreamFileJsonUnmarshaller implements Unmarshaller<StreamFile, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("fileId", targetDepth)) {
                    context.nextToken();
                    streamFile.setFileId(context.getUnmarshaller(Integer.class).unmarshall(context));
                }
                if (context.testExpression("s3Location", targetDepth)) {
                    context.nextToken();
                    streamFile.setS3Location(S3LocationJsonUnmarshaller.getInstance().unmarshall(context));
                }
