        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("ServerId", targetDepth)) {
                    context.nextToken();
                    describeUserResult.setServerId(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("User", targetDepth)) {
                    context.nextToken();
                    describeUserResult.setUser(DescribedUserJsonUnmarshaller.getInstance().unmarshall(context));
                }
