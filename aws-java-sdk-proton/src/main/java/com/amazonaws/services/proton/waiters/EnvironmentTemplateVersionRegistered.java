...
class EnvironmentTemplateVersionRegistered {
    ...
    static class IsDRAFTMatcher extends WaiterAcceptor<GetEnvironmentTemplateVersionResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"DRAFT\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new JmesPathSubExpression(new JmesPathField("environmentTemplateVersion"), new JmesPathField("status"));
         ...
         */
        @Override
        public boolean matches(GetEnvironmentTemplateVersionResult result) {
    ...
    static class IsPUBLISHEDMatcher extends WaiterAcceptor<GetEnvironmentTemplateVersionResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"PUBLISHED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
    ...
    static class IsREGISTRATION_FAILEDMatcher extends WaiterAcceptor<GetEnvironmentTemplateVersionResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"REGISTRATION_FAILED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
