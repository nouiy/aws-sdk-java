...
class RepositoryAssociationSucceeded {
    ...
    static class IsAssociatedMatcher extends WaiterAcceptor<DescribeRepositoryAssociationResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"Associated\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new JmesPathSubExpression(new JmesPathField("RepositoryAssociation"), new JmesPathField("State"));
         ...
         */
        @Override
        public boolean matches(DescribeRepositoryAssociationResult result) {
    ...
    static class IsAssociatingMatcher extends WaiterAcceptor<DescribeRepositoryAssociationResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"Associating\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
