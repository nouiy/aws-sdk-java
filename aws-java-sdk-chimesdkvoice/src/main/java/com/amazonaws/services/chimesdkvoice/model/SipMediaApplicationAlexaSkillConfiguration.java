/*
 * Copyright 2018-2023 Amazon.com, Inc. or its affiliates. All Rights Reserved.
 * 
 * Licensed under the Apache License, Version 2.0 (the "License"). You may not use this file except in compliance with
 * the License. A copy of the License is located at
 * 
 * http://aws.amazon.com/apache2.0
 * 
 * or in the "license" file accompanying this file. This file is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR
 * CONDITIONS OF ANY KIND, either express or implied. See the License for the specific language governing permissions
 * and limitations under the License.
 */
package com.amazonaws.services.chimesdkvoice.model;

import java.io.Serializable;
import javax.annotation.Generated;
import com.amazonaws.protocol.StructuredPojo;
import com.amazonaws.protocol.ProtocolMarshaller;

/**
 * <p>
 * The Alexa Skill configuration of a SIP media application.
 * </p>
 * 
 * @see <a
 *      href="http://docs.aws.amazon.com/goto/WebAPI/chime-sdk-voice-2022-08-03/SipMediaApplicationAlexaSkillConfiguration"
 *      target="_top">AWS API Documentation</a>
 */
@Generated("com.amazonaws:aws-java-sdk-code-generator")
public class SipMediaApplicationAlexaSkillConfiguration implements Serializable, Cloneable, StructuredPojo {

    /**
     * <p>
     * The status of the Alexa Skill configuration.
     * </p>
     */
    private String alexaSkillStatus;
    /**
     * <p>
     * The ID of the Alexa Skill configuration.
     * </p>
     */
    private java.util.List<String> alexaSkillIds;

    /**
     * <p>
     * The status of the Alexa Skill configuration.
     * </p>
     * 
     * @param alexaSkillStatus
     *        The status of the Alexa Skill configuration.
     * @see AlexaSkillStatus
     */

    public void setAlexaSkillStatus(String alexaSkillStatus) {
        this.alexaSkillStatus = alexaSkillStatus;
    }

    /**
     * <p>
     * The status of the Alexa Skill configuration.
     * </p>
     * 
     * @return The status of the Alexa Skill configuration.
     * @see AlexaSkillStatus
     */

    public String getAlexaSkillStatus() {
        return this.alexaSkillStatus;
    }

    /**
     * <p>
     * The status of the Alexa Skill configuration.
     * </p>
     * 
     * @param alexaSkillStatus
     *        The status of the Alexa Skill configuration.
     * @return Returns a reference to this object so that method calls can be chained together.
     * @see AlexaSkillStatus
     */

    public SipMediaApplicationAlexaSkillConfiguration withAlexaSkillStatus(String alexaSkillStatus) {
        setAlexaSkillStatus(alexaSkillStatus);
        return this;
    }

    /**
     * <p>
     * The status of the Alexa Skill configuration.
     * </p>
     * 
     * @param alexaSkillStatus
     *        The status of the Alexa Skill configuration.
     * @return Returns a reference to this object so that method calls can be chained together.
     * @see AlexaSkillStatus
     */

    public SipMediaApplicationAlexaSkillConfiguration withAlexaSkillStatus(AlexaSkillStatus alexaSkillStatus) {
        this.alexaSkillStatus = alexaSkillStatus.toString();
        return this;
    }

    /**
     * <p>
     * The ID of the Alexa Skill configuration.
     * </p>
     * 
     * @return The ID of the Alexa Skill configuration.
     */

    public java.util.List<String> getAlexaSkillIds() {
        return alexaSkillIds;
    }

    /**
     * <p>
     * The ID of the Alexa Skill configuration.
     * </p>
     * 
     * @param alexaSkillIds
     *        The ID of the Alexa Skill configuration.
     */

    public void setAlexaSkillIds(java.util.Collection<String> alexaSkillIds) {
        if (alexaSkillIds == null) {
            this.alexaSkillIds = null;
            return;
        }

        this.alexaSkillIds = new java.util.ArrayList<String>(alexaSkillIds);
    }

    /**
     * <p>
     * The ID of the Alexa Skill configuration.
     * </p>
     * <p>
     * <b>NOTE:</b> This method appends the values to the existing list (if any). Use
     * {@link #setAlexaSkillIds(java.util.Collection)} or {@link #withAlexaSkillIds(java.util.Collection)} if you want
     * to override the existing values.
     * </p>
     * 
     * @param alexaSkillIds
     *        The ID of the Alexa Skill configuration.
     * @return Returns a reference to this object so that method calls can be chained together.
     */

    public SipMediaApplicationAlexaSkillConfiguration withAlexaSkillIds(String... alexaSkillIds) {
        if (this.alexaSkillIds == null) {
            setAlexaSkillIds(new java.util.ArrayList<String>(alexaSkillIds.length));
        }
        for (String ele : alexaSkillIds) {
            this.alexaSkillIds.add(ele);
        }
        return this;
    }

    /**
     * <p>
     * The ID of the Alexa Skill configuration.
     * </p>
     * 
     * @param alexaSkillIds
     *        The ID of the Alexa Skill configuration.
     * @return Returns a reference to this object so that method calls can be chained together.
     */

    public SipMediaApplicationAlexaSkillConfiguration withAlexaSkillIds(java.util.Collection<String> alexaSkillIds) {
        setAlexaSkillIds(alexaSkillIds);
        return this;
    }

    /**
     * Returns a string representation of this object. This is useful for testing and debugging. Sensitive data will be
     * redacted from this string using a placeholder value.
     *
     * @return A string representation of this object.
     *
     * @see java.lang.Object#toString()
     */
    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        if (getAlexaSkillStatus() != null)
            sb.append("AlexaSkillStatus: ").append(getAlexaSkillStatus()).append(",");
        if (getAlexaSkillIds() != null)
            sb.append("AlexaSkillIds: ").append("***Sensitive Data Redacted***");
        sb.append("}");
        return sb.toString();
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (obj == null)
            return false;

        if (obj instanceof SipMediaApplicationAlexaSkillConfiguration == false)
            return false;
        SipMediaApplicationAlexaSkillConfiguration other = (SipMediaApplicationAlexaSkillConfiguration) obj;
        if (other.getAlexaSkillStatus() == null ^ this.getAlexaSkillStatus() == null)
            return false;
        if (other.getAlexaSkillStatus() != null && other.getAlexaSkillStatus().equals(this.getAlexaSkillStatus()) == false)
            return false;
        if (other.getAlexaSkillIds() == null ^ this.getAlexaSkillIds() == null)
            return false;
        if (other.getAlexaSkillIds() != null && other.getAlexaSkillIds().equals(this.getAlexaSkillIds()) == false)
            return false;
        return true;
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int hashCode = 1;

        hashCode = prime * hashCode + ((getAlexaSkillStatus() == null) ? 0 : getAlexaSkillStatus().hashCode());
        hashCode = prime * hashCode + ((getAlexaSkillIds() == null) ? 0 : getAlexaSkillIds().hashCode());
        return hashCode;
    }

    @Override
    public SipMediaApplicationAlexaSkillConfiguration clone() {
        try {
            return (SipMediaApplicationAlexaSkillConfiguration) super.clone();
        } catch (CloneNotSupportedException e) {
            throw new IllegalStateException("Got a CloneNotSupportedException from Object.clone() " + "even though we're Cloneable!", e);
        }
    }

    @com.amazonaws.annotation.SdkInternalApi
    @Override
    public void marshall(ProtocolMarshaller protocolMarshaller) {
        com.amazonaws.services.chimesdkvoice.model.transform.SipMediaApplicationAlexaSkillConfigurationMarshaller.getInstance().marshall(this,
                protocolMarshaller);
    }
}
