...
public class ScheduleRunResultJsonUnmarshaller implements Unmarshaller<ScheduleRunResult, JsonUnmarshallerContext> {

    public ScheduleRunResult unmarshall(JsonUnmarshallerContext context) throws Exception {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("run", targetDepth)) {
                    context.nextToken();
                    scheduleRunResult.setRun(RunJsonUnmarshaller.getInstance().unmarshall(context));
                }
