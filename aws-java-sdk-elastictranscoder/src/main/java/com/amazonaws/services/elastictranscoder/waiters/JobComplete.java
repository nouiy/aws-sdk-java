...
class JobComplete {
    ...
    static class IsCompleteMatcher extends WaiterAcceptor<ReadJobResult> {
        ...
        private static final JmesPathExpression ast = new JmesPathSubExpression(new JmesPathField("Job"), new JmesPathField("Status"));
         ...
         */
        @Override
        public boolean matches(ReadJobResult result) {
    ...
    static class IsCanceledMatcher extends WaiterAcceptor<ReadJobResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"Canceled\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
    ...
    static class IsErrorMatcher extends WaiterAcceptor<ReadJobResult> {
        private static final JsonNode expectedResult;
