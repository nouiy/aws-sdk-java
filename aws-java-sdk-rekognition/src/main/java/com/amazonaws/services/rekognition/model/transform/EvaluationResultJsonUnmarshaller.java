        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("F1Score", targetDepth)) {
                    context.nextToken();
                    evaluationResult.setF1Score(context.getUnmarshaller(Float.class).unmarshall(context));
                }
                if (context.testExpression("Summary", targetDepth)) {
                    context.nextToken();
                    evaluationResult.setSummary(SummaryJsonUnmarshaller.getInstance().unmarshall(context));
                }
