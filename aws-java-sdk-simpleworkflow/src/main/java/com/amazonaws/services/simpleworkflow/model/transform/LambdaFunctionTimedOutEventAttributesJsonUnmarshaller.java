...
public class LambdaFunctionTimedOutEventAttributesJsonUnmarshaller implements Unmarshaller<LambdaFunctionTimedOutEventAttributes, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("scheduledEventId", targetDepth)) {
                    context.nextToken();
                    lambdaFunctionTimedOutEventAttributes.setScheduledEventId(context.getUnmarshaller(Long.class).unmarshall(context));
                }
                if (context.testExpression("startedEventId", targetDepth)) {
                    context.nextToken();
                    lambdaFunctionTimedOutEventAttributes.setStartedEventId(context.getUnmarshaller(Long.class).unmarshall(context));
                }
                if (context.testExpression("timeoutType", targetDepth)) {
                    context.nextToken();
                    lambdaFunctionTimedOutEventAttributes.setTimeoutType(context.getUnmarshaller(String.class).unmarshall(context));
                }
    ...
    public static LambdaFunctionTimedOutEventAttributesJsonUnmarshaller getInstance() {
        if (instance == null)
            instance = new LambdaFunctionTimedOutEventAttributesJsonUnmarshaller();
        return instance;
    }
}
