...
class EnvironmentExists {
    ...
    static class IsReadyMatcher extends WaiterAcceptor<DescribeEnvironmentsResult> {
    ...
    static class IsLaunchingMatcher extends WaiterAcceptor<DescribeEnvironmentsResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"Launching\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
