...
public class DeploymentReadyOptionJsonUnmarshaller implements Unmarshaller<DeploymentReadyOption, JsonUnmarshallerContext> {
        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("actionOnTimeout", targetDepth)) {
                    context.nextToken();
                    deploymentReadyOption.setActionOnTimeout(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("waitTimeInMinutes", targetDepth)) {
                    context.nextToken();
                    deploymentReadyOption.setWaitTimeInMinutes(context.getUnmarshaller(Integer.class).unmarshall(context));
                }
