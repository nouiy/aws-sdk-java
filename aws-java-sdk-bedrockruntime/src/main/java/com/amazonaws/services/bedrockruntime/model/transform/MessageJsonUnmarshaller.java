        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("role", targetDepth)) {
                    context.nextToken();
                    message.setRole(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("content", targetDepth)) {
                    context.nextToken();
                    message.setContent(new ListUnmarshaller<ContentBlock>(ContentBlockJsonUnmarshaller.getInstance())
