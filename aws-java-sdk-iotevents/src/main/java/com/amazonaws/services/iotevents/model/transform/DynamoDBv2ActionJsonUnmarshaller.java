        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("tableName", targetDepth)) {
                    context.nextToken();
                    dynamoDBv2Action.setTableName(context.getUnmarshaller(String.class).unmarshall(context));
                }
                if (context.testExpression("payload", targetDepth)) {
                    context.nextToken();
                    dynamoDBv2Action.setPayload(PayloadJsonUnmarshaller.getInstance().unmarshall(context));
                }
