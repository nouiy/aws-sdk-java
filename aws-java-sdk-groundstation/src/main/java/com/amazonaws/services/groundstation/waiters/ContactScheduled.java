...
class ContactScheduled {
    ...
    static class IsFAILED_TO_SCHEDULEMatcher extends WaiterAcceptor<DescribeContactResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"FAILED_TO_SCHEDULE\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }

        private static final JmesPathExpression ast = new JmesPathField("contactStatus");
         ...
         */
        @Override
        public boolean matches(DescribeContactResult result) {
    ...
    static class IsSCHEDULEDMatcher extends WaiterAcceptor<DescribeContactResult> {
        private static final JsonNode expectedResult;

        static {
            try {
                expectedResult = ObjectMapperSingleton.getObjectMapper().readTree("\"SCHEDULED\"");
            } catch (IOException ioe) {
                throw new RuntimeException(ioe);
            }
        }
